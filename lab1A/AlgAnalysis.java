...
public class AlgAnalysis {
	...
	public static void arraySortRuntime(int count, int maxSize){
		...
		for (int i = 0; i <= count;i++) {
			...
			System.out.println("Time taken by Arrays.sort : " + (runTimeInNanoSeconds));
			System.out.println("N:"+ myArray.length + ", T(n): " + (runTimeInNanoSeconds)  + " nanosecs, T(n)/(n*n): " + runTimeInNanoSeconds/(myArray.length * myArray.length) + "") ;
			avgTime = avgTime + runTimeInNanoSeconds;
			randomizeArray(myArray);
		}
		System.out.println("The average run time of Arrays.sort is: " + nanoToSeconds(avgTime/count));
	}
	...
	public static long unique1Runtime(int n){
		...
		int[] array1 = genArray(n);
		long runTimeInNanoSeconds = 0;
		for (int i = 0; i <= n; i++) {
			long nanoTime1 = System.nanoTime();
			//Arrays.sort(array1);
			Unique1.unique1(array1);
			long nanoTime2 = System.nanoTime();
			runTimeInNanoSeconds = nanoTime2 - nanoTime1;
			System.out.println("Time taken by Unique1" + (runTimeInNanoSeconds));
	...
	public static long unique2Runtime(int n){
		...
		long nanoTime1 = System.nanoTime();
		Unique1.unique1(array2);
		long nanoTime2 = System.nanoTime();
		long runTimeInNanoSeconds = nanoTime2 - nanoTime1;
		System.out.println("Time taken by Unique1: " + nanoToSeconds(runTimeInNanoSeconds) );
		return 0;
	}
	...
	/**
	 * Main method
	 * Provides prompts for all experiments0
	 */
