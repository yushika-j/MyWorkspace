...
def min_matrix(m):
     ...
     Precondition: m represents a matrix all of whose elements are numbers and
                   and m has at least 1 row and 1 columb'''
...
def make_matrix_random(a,b, x, y):
     ...
     m = make_matrix(a,b,int)
     
     if x<=y:
          for rows in range(a):
               #m.append([])
               for column in range(b):
                    m[rows][column]=(random.randrange(x,y))
               print()
...
def sum_above_diagonal(m):
     ...
     for rows in range(0,len(m)):
          for columns in range(rows,len(m)):
               total = total + m[rows][columns]
     return total
...
def max_over_all_even_cols(m):
     ...
     for row in range(0,len(m)):
          for col in range(0,len(m[row]),2):
               if m[row][col] > max:
                    max = m[row][col]
     return max
...
def index_of_max_sum_row(m):
     ...
     for row in range(len(m)):
          lst.append(sum(m[row]))
     maximum = max(lst)
     return lst.index(maximum)
# main
# print(make_matrix_random(3,4, 1, 6))
