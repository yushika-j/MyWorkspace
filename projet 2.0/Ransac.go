...
func main() {
	...
	// // Wait group to synchronize pipeline components
	wg := &sync.WaitGroup{}

	// Random point generator
	randomPointCh := randomPointGenerator(wg, stop, points)

	// Triplet of points generator
	tripletCh := tripletPointsGenerator(stop, randomPointCh)

	// TakeN
	takeNCh := takeN(wg, stop, tripletCh, numberOfIterations)

	// Plane estimator
	planeCh := planeEstimator(*wg, stop, takeNCh)

	// Supporting point finder
	supportCh := supportingPointFinder(wg, stop, planeCh, points, eps)

	// Fan in
	fanInCh := FanIn(wg, stop, <-chan *[]Plane3DwSupport)

	// Dominant plane identifier
	dominantPlane := &Plane3DwSupport{}
	DominantPlaneIdentifier(wg, stop, fanInCh, dominantPlane)

	// Start pipeline
	wg.Add(4)
	for key := range planeEstimator(&wg, stop, takeN(stop, tripletPointsGenerator(&wg, stop, randomPointGenerator(&wg, stop, points)), numberOfIterations, GetPlane)) {
	...
	fanOut := 8800
	wg.Add(1)
	filter := make([]<-chan Plane3DwSupport, fanOut)
	...
	go func() {
		...
		for i := 0; i < numberOfIterations; i++ {
			...
			case <-stop:
				return
			default:
				fmt.Printf("Dominant plane: %v\n", dominantPlane.Plane3D)
				fmt.Printf("Supporting points: %d\n", dominantPlane.SupportSize)
			}
		}
	}()
